...
struct Device: DeviceProtocol {
    ...
    func networkType() -> String {
        ...
        if #available(iOS 12.0, *) {
            carrierType = networkInfo.serviceCurrentRadioAccessTechnology?.values.first
        } else {
            carrierType = networkInfo.currentRadioAccessTechnology
        }
        switch carrierType {
        ...
        case CTRadioAccessTechnologyLTE:
            return "4g"
        default:
            return self.check5G(carrierType)
        }
    }
    
    private func check5G(_ carrierType: String?) -> String {
